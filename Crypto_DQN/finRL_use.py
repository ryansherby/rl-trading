...
DATA_DIR    = "./Crypto_DQN/data"
OUTPUT_ROOT = "./Crypto_DQN/outputs"
...
for model in ["ppo", "ddpg", "sac"]:
    for idx, tic in enumerate(tickers):
        
        TARGET_ROUTE = OUTPUT_ROOT+f"/{model}/{tic}"
        
        os.makedirs(TARGET_ROUTE, exist_ok=True)
        ...
        if idx == 0:
            ...
            model_ppo = agent.get_model(model,model_kwargs = model_params[model])

            # set up logging once
            logger = configure(TARGET_ROUTE, ["stdout", "csv", "tensorboard"])
    ...
    # g) checkpoint ,pde;
    ckpt = os.path.join(OUTPUT_ROOT, f"{model}_model.zip")
    model_ppo.save(ckpt)
    ...
    # Path to save
    out_path = os.path.join(OUTPUT_ROOT, f"{model}_all_tickers_equity_curves.png")
