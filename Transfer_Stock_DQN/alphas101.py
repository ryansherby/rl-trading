...
#                             columns=self.close.columns)
    ...
    def alpha023(self):
        ...
        alpha.loc[cond,'close'] = -1 * delta(self.high, 2)
        return alpha
    ...
    def alpha071(self):
        ...
        df['max'] = df.apply(lambda row: max(row['p1'],row['p2']), axis=1)
        return df['max']
    ...
    def alpha077(self):
        ...
        df=pd.DataFrame({'p1':p1,'p2':p2})
        df['min'] = df.apply(lambda row: min(row['p1'],row['p2']), axis=1)
        return df['min']
